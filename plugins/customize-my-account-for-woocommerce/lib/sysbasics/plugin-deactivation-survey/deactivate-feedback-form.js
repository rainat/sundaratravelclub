		...
		var element = $('\
			<div class="sysbasics-deactivate-dialog" data-remodal-id="' + plugin.slug + '">\
				<form>\
					...
					<br>\
					<br>\
					<p class="sysbasics-theme-info" style="display:none;">' + strings.theme_name + '&emsp; : &emsp;<input name="theme"  class="sysbasics-theme-name" value ="' + strings.theme + '" placeholder="' + strings.theme_name + '"/></p>\
					<br>\
					<p class="sysbasics-theme-info" style="display:none;">\
					' + strings.theme_fix + '\
					</p>\
					<br>\
					<p class="sysbasics-deactivate-dialog-buttons">\
					   <input type="hidden" class="elementor_mode" value="' + strings.elementor_mode + '"/>\
						<input type="submit" class="button confirm" value="' + strings.skip_and_deactivate + '"/>\
						<a target="_blank" href="https://wordpress.org/support/plugin/customize-my-account-for-woocommerce/#new-topic-0" class="button button-success">' + strings.contact_support + '</a>\
		...
		if($(element).find("input[name='reason']:checked").length)
		{
			...
			$.ajax({
				...
				data:		{
                    ...
                    theme   : $(".sysbasics-theme-name").val(),
                    elementor_mode : $(".elementor_mode").val()
