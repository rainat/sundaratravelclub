...
const menu = [
	...
	},
	{
		...
		title: (
			<span>
				Click{" "}
				<a className="text-[#BEB29A]" href={myaccountobj.url_logout}>
					here
				</a>{" "}
				to logout'
			</span>
...
const Control = ({ children, ...props }: ControlProps) => {
	...
	return (
		<components.Control {...props}>
			<img className="mx-2 " src={value.icon} style={{ width: 15 }} />
			{children}
		</components.Control>
	);
};
	...
	return (
		<div
			...
			className={classNames(
				"cursor-pointer h-[48px] flex items-center gap-4",
				{
					"font-bold": active,
					"text-[#8C8C8C]": !active,
		...
		>
			...
			<div className="w-full items-center flex justify-between">
				...
				<span
					className={classNames(
						{ hidden: !active },
						"w-2 h-2  rounded-full bg-[#A87C51]",
					)}
				></span>
			</div>
		</div>
			...
			<div className="flex flex-col gap-4">
				...
				<div className="md:hidden flex flex-col content-center">
					<Select
						options={menu}
						defaultValue={menu[0]}
						onChange={(val) => gomenu(val)}
						components={{ Option: IconOption, Control } as any}
						classNames={{
