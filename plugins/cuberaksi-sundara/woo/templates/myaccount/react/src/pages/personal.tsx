...
import { ChangeEvent, ReactNode, createRef, useEffect, useRef, useState } from "react"
...
export function Personal() {
	...
	const [fileState, __setFileState] = useState<IFileState>({ file: null, imageSrc: null })
	...
	useEffect(() => {
		if (profileQuery.data) {
			...
			__setFileState({ ...fileState, imageSrc: profileQuery.data.profile_photo })
		}
	...
	const handlefilechange = (event: ChangeEvent<HTMLInputElement>) => {
		...
		if (event.target.files) {
			...
			let localImageUrl = imageFile ? URL.createObjectURL(imageFile) : null

			__setFileState({ ...fileState, file: event.target.files[0], imageSrc: localImageUrl });
	...
	const getImageUrl = () => {
		return fileState.imageSrc ? fileState.imageSrc as string : imgsrcdulu
	}
	...
	return (
		<div id="personal_tab">
			<Card className="md:w-[717px] animate-bounced ">
				...
				<div className="flex gap-6 mt-8">
					...
					<div className="mt-2">
						...
						)} disabled={!editMode} onClick={() => { if (editMode) fileref.current?.click() }}>Change </button>
					</div>
				</div>
				...
				<div className="mt-4 md:w-[661px] border-b-[1px] border-[#C7C7C7]"></div>
