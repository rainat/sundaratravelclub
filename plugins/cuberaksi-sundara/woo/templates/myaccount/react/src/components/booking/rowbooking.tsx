...
export function RowBooking({ item, section }: TRowBookings) {
	...
	const ProcessContent = () => {
		...
		const convertStatusText = (status: string) => {
			...
			if (status === 'Paid') statusText = 'Pending'
			if (status === 'Confirmed') statusText = 'On Progress'
			if (status === 'Rejected') statusText = 'Cancelled'
			return statusText
		}

		return (<>
			<Button className={classNames("rounded-full", getStatusClass(item.status))}>{convertStatusText(item.status)}</Button>
		</>)
	}
