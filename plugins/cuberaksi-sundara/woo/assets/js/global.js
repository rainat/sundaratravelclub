...
jQuery(document).ready(($) => {
	...
	$("#yith-form-fake").click(() => {
		...
		$(".yith_wcbk_booking_product_form_widget").css({
			top: "15%",
			left: "40%",
		});
	});
		...
		if ($(".woocommerce-notices-wrapper").children().length > 0) {
			if (
				$(".woocommerce-notices-wrapper .woocommerce-message")
					.text()
					.includes("Request for booking confirmation sent")
			) {
				location.href =
	...
	function generateTime() {
		if (typeof productme !== "undefined") {
			...
			let checkin = $(
				".yith-booking-checkin > .yith-booking-meta__value",
			);
			...
			let checkout = $(
				".yith-booking-checkout > .yith-booking-meta__value",
			);
	...
	if ($("span.yith-wcbk-booking-duration__label").length)
		var duration = Number(
			$("span.yith-wcbk-booking-duration__label")[0].textContent.replace(
				" days",
				"",
			),
		);
	...
	function renderDetailPrices() {
		if (
			document.querySelectorAll(".woocommerce-Price-amount.amount")
				.length > 0
		) {
	...
	function renderIncludeTaxText() {
		if ($(".itaxfee").length <= 0)
			$(".yith_wcbk_booking_product_form_widget p.price").append(
