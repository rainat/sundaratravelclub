(function ($) {
	$(document).ready(() => {
		$(
			"div.elementor-element.elementor-element-a305e87.elementor-widget.elementor-widget-heading > div > h2",
		).css("display", "none");
		if (typeof galleries !== undefined) {
			...
			function doPopupGallery(slideData) {
				if (elementorProFrontend.modules.popup) {
					...
					let itenary =
						galleries[`day${slideData.day}`][`${slideData.gal}`].title;
         ...
         <img src="${img.thumbnail}" alt="" />
					...
					let content =
						`<section id="main-slider" class="splide" aria-label="My Sundara Gallery">
      <div class="splide__track">
        <ul class="splide__list">` +
						populateImages(0, "full") +
						`</ul>
      </div>
    </section>

    <ul id="thumbnails" class="thumbnails">` +
						populateImages(0, "thumbnail") +
						`</ul>`;
			...
			function populate_itenary(index) {
				...
				function populate_gallery_link(gallery, day, idx) {
					if (gallery)
						return `<a class='gallery-link ' data-slide='{"day":"${day}","gal":"${idx}"}'  href='javascript:void(0);'>View Imaged Gallery > </a>`;
					return "";
				}
    ...
    <div class="ht-addinfo__col ht-addinfo__img">${img}</div> <div class="ht-addinfo__col ht-addinfo__desc">
        <p class="ht-addinfo__title">${itm.title}</p>` +
							populate_description(itm.description) +
							populate_gallery_link(itm.gallery, index, idx) +
							"</div></div>";
					});
					return results;
				}
			...
			//timeline
			// console.log({galleries})
			const gal = $('.timeline[data-gol="itenary"] .container .content').each(
				(index, el) => {
			...
			// 		populate_itenary(index + 1) +
			// 		"</div>";
			// 	if (galleries[`day${index + 1}`]) $(el).html(children);
			// 	//
			// });
