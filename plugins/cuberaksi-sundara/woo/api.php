...
class Cuberaksi_Api
{
    ...
    public function init_ajax()
    {
        ...
        add_action('wp_enqueue_scripts', function () {
            wp_localize_script('jquery', 'sundara', ['ajaxurl' => admin_url('admin-ajax.php')]);
        });
    }
    ...
    public function init_api()
    {
        ...
        add_action('rest_api_init', function () {
            register_rest_route('sundara/v1', '/bookings', [
                'methods' => 'GET',
            ...
            register_rest_route('sundara/v1', '/slots', [
                'methods' => 'GET',
            ...
            register_rest_route('sundara/v1', '/extrainfo', [
                'methods' => 'GET',
                'callback' => [$this, 'api_get_extra_info'],
                'permission_callback' => '__return_true',
                // 'permission_callback' => function () {
                // return current_user_can('edit_others_posts');
                // }
    ...
    public function api_get_extra_info(\WP_REST_Request $request)
    {
        $q = $request->get_query_params();
        if (isset($q['orderid'])) {
            $order = wc_get_order($q['orderid']);

            $results = [
                'orderId' => $q['orderid'],

                'mailingAddress' => $order->get_meta('_cuber_extra_mailing_address'),
                'birth' => $order->get_meta('_cuber_extra_date_birth'),
                'dietary' => $order->get_meta('_cuber_extra_dietary'),
                'medicalIssue' => $order->get_meta('_cuber_extra_medical_issue'),
                'emergencyContactName' => $order->get_meta('_cuber_extra_emergency_contact_name'),
                'emergencyContactEmail' => $order->get_meta('_cuber_extra_emergency_contact_name'),
                'hearFrom' => $order->get_meta('_cuber_extra_hear_from'),
                'anythingElse' => $order->get_meta('_cuber_extra_anything_else'),
            ];

            return new \WP_REST_Response($results);
        }

        return new \WP_REST_Response(['msg' => 'error'], 400);
    }
    ...
    public function api_post_slots(\WP_REST_Request $request)
    {
    ...
    public function ajax_get_bookings()
    {
        ...
        switch ($status_request) {
            ...
            case 'Awaiting-Approval':
                $status_sql = "AND (p.post_status = 'bk-pending-confirm' OR p.post_status = 'bk-confirmed' OR p.post_status = 'bk-paid' ) ";
                break;
            default:
                $status_sql = '';
        }
        ...
        if (!is_wp_error($temp)) {
            foreach ($temp as $row) {
                ...
                if ($row['post_status'] === 'bk-paid') {
                    $section = 'awaitingApproval'; // Paid
                    $status = 'Paid';
                }
                if ($row['post_status'] === 'bk-unpaid') {
                    $section = 'awaitingApproval'; // Unpaid
                    $status = 'Unpaid';
                }
                if ($row['post_status'] === 'bk-confirmed') {
                    $section = 'awaitingApproval'; // Confirmed
                    $status = 'Confirmed';
                }
    ...
    public function ajax_get_bookings_nopriv()
    {
    ...
    // ajax
    public function ajax_get_profile_nopriv()
    {
        echo 'You must log in';
        exit;
    }
    ...
    public function user_profile_post_noprivs()
    {
        echo 'You must log in';
        exit;
    }
